<div class="container">
    <div class="row footer w-100">
        <div class="col-12 col-sm-2">
            <img alt="logo" class="img-fluid" src="img/logo.png">
        </div>
        <div class="col-12 col-sm-7">
            <div class="linkjes">
                <a class="nav-item text-reset" href="band.php">Band</a>
                <a class="nav-item text-reset" href="albums.php">Albums</a>
                <a class="nav-item text-reset" href="nieuws.php">Nieuws</a>
                <a class="nav-item text-reset" href="open-air.php">Sabaton Open Air</a>
                <a class="nav-item text-reset" href="Cruise.php">Sabaton Cruise</a>
                <a class="nav-item text-reset" href="Contact.php">Contact</a>
            </div>
        </div>

        <div class="col-12 col-sm-3 contact" >
            <div class="col-12 col-sm-3 contact" >
                <h4>Social</h4>
                <a href="https://www.youtube.com/channel/UCjQhd1APsd5NQhiVZV7GYzg" class="fab fa-youtube"></a>
                <a href="https://www.facebook.com/sabaton/" class="fab fa-facebook-square"></a>
                <a href="https://twitter.com/sabaton" class="fab fa-twitter-square"></a>
            </div>
        </div>
    </div>

</div>
